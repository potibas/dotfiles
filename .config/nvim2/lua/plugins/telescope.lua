...
local map = function(mapping, description, callback)
  return {
    mapping,
    function() callback(require('telescope.builtin')) end,
    desc = description
  }
end
...
return {
  ...
  kues = {
    map('<c-p>', 'Find Files', function(b) b.find_files({ hidden = true }) end),
    map('<c-f>', 'Live Grep', function(b) b.live_grep() end),
  }
}
